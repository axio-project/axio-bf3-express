...
#define MAX_BUFF_SIZE	(2560)	/* Maximum DOCA buffer size */
...
//	strncpy(src_buf_data, cfg->send_string, MAX_BUFF_SIZE + 1);
        result = read_file(cfg->sendfile_path, (char **)&((&resources)->sendfile_buf),
                                &(&resources)->sendfile_size);
	strncpy(src_buf_data, resources.sendfile_buf, MAX_BUFF_SIZE + 1);
	...
	DOCA_LOG_INFO("Submitting DOCA RDMA send job that sends \"%s\" to receiver", resources.sendfile_buf);
