...
function usage() {
        ...
        Configures IPsec connection between two BlueFields with HW full offload.
...
--preshared_key=swordfish
        ...
        --offload_mode=MODE                   The hardware offload mode, can be none or full. If this parameter isn't
                                              passed then by default the mode is full.
...
function config_offload() {
        ipsec_mode_folder="/sys/class/net/p${port_num}/compat/devlink/ipsec_mode"

        if [ -f $ipsec_mode_folder ]; then
                ipsec_current_mode=$(cat $ipsec_mode_folder)
        else
                err "Can't show IPsec offload mode, exiting."
                # Exit
                exit 1
        fi

        if [[ $1 -eq 1 ]] ; then
                echo "Configuring IPsec HW offload mode to \"full\"."
        ...
        # Check if ipsec_mode is correct.
        if [[ -f /sys/class/net/p${port_num}/compat/devlink/ipsec_mode ]]; then
                ipsec_current_mode=$(cat /sys/class/net/p${port_num}/compat/devlink/ipsec_mode)
                diff  <(echo $ipsec_wanted_mode) <(echo $ipsec_current_mode) > /dev/null 2>&1
                if (( $? == 0 )); then
                        echo "Configuration succeeded. IPsec HW offload mode is \"$ipsec_current_mode\"."
                else
                        err "IPSec offload configuration failed. IPsec HW offload mode is \"$ipsec_current_mode\", mode should be \"$ipsec_wanted_mode\"."
                        # Re-enable regex.
                ...
                err "Can't show IPsec offload mode."
                # Re-enable regex.
                echo "Re-enabling regex."
                systemctl restart mlx-regex
                # Exit
...
# This function reverts IPsec rules (if they exist), disables IPsec full offload mode,
# and deletes local certificates and key files (if they exist).
...
function configure_parameters() {
        ...
        if [[ -z $offload_mode ]]; then
                ...
                if [[ "$offload_mode" != "full" && "$offload_mode" != "none" ]]; then
                        err "Illegal offload mode $offload_mode, offload mode must be full or none. Exiting."
                        # Exit
                        exit 1
                fi
        fi
